...
def split_data_by_days(data, val_days, test_days, interval=15):
    '''
    :param data: [B, *]
    :param val_days:
    :param test_days:
...
#     x_np = X
#     align_x = align_np(x_np)
#     # print('align_x', align_x.shape)
#     cuda = True if torch.cuda.is_available() else False
#     TensorFloat = torch.cuda.FloatTensor if cuda else torch.FloatTensor
#     X, Y, align_x = TensorFloat(X), TensorFloat(Y), TensorFloat(align_x)

    
#     data = torch.utils.data.TensorDataset(X, Y, align_x)
...
def get_dataloader(args, normalizer = 'std', tod=False, dow=False, weather=False, single=True):
    #load raw st dataset
    data = load_st_dataset(args.dataset, args.input_dim, args.num_nodes)        # B, N, D
    #normalize st data
    ...
    #add time window
    step_one = True # 得到数据时按照进一步一个数据。
    x_tra, y_tra = Add_Window_Horizon(data_train, args.lag, args.horizon, single, step_one)
    x_val, y_val = Add_Window_Horizon(data_val, args.lag, args.horizon, single, step_one)
    x_test, y_test = Add_Window_Horizon(data_test, args.lag, args.horizon, single, step_one)
